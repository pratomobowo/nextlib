...
async function handleGet(
  ...
  // 4b. Lazily ensure Ed25519 keypair exists (no rotation on ZIP download —
  // public key is stable; rotating requires a separate endpoint per Task 7).
  const { publicKey } = await ensureTenantKeypair(id);
...
function buildEnvFile(opts: {
  ...
  const { apiSecret, publicKey, tenantId, cloudBaseUrl, tenantSlug } = opts;
  const today = new Date().toISOString().slice(0, 10);
...
#
# Auth scheme: Ed25519 signatures (v2). NEXTLIB_PUBLIC_KEY is the SaaS
# public key for verifying signed requests (X-NextLib-Timestamp +
# X-NextLib-Signature headers). NEXTLIB_TOKEN_SECRET is the legacy HMAC
# shared secret (v1) — kept here temporarily for backward compat with
# older SaaS deployments; safe to delete once v1 is fully retired.

# Ed25519 public key (base64, 32 bytes). Verifying-only — safe to share.
NEXTLIB_PUBLIC_KEY=${publicKey}

# HMAC-SHA256 secret for /api/v1/nextlib/* token validation (legacy v1)
